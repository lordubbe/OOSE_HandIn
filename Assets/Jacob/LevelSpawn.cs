...
public class LevelSpawn : MonoBehaviour
{
    ...
    //Enemy strength multiplayer
    public float enemyStrength = 1.0f;
            ...
            for (int x = 0; x < MAX_LEVEL_WIDTH; x++)
            {
                for (int y = 0; y < MAX_LEVEL_HEIGHT; y++)
                {
                    if (levelMatrix[x, y] != null && levelMatrix[x, y].canSpawnEnemies && Random.Range(0, 100) < enemySpawnFreq)
                    {
                        ...
                        //Jacob says: "GRRRRRR!!!"
                        if (levelMatrix[x, y].tileMesh.childCount != null && levelMatrix[x, y].tileMesh.childCount < 1)
                        {//only spawn enemy if there is not something else spawned there
                            Transform enemy = (Transform)Instantiate(enemyObject, new Vector3(x * tileWidth, 1, y * tileHeight), Quaternion.identity);
                            enemy.name = "lol a frog";//

                            //change the stats of the monsters
...
*/
        ...
        for (int i = 0; i < numberOfRooms * 2; i += 2)
        {
            ...
            if (roomArray[i] + roomSizeArray[i] > MAX_LEVEL_WIDTH)
            {//if the x coordinate + the room width exceeds the level width (Out Of Bounds [OOB])
            ...
            if (roomArray[i + 1] + roomSizeArray[i + 1] > MAX_LEVEL_HEIGHT)
            {//same for height
        ...
        int roomCount = 0;
        for (int k = 0; k < roomArray.Length; k += 2)
        {//iterate through each room
            ...
            for (int l = roomArray[k]; l < roomArray[k] + roomSizeArray[k]; l++)
            {//width of room spawned from the x position 
                for (int m = roomArray[k + 1]; m < roomArray[k + 1] + roomSizeArray[k + 1]; m++)
                {//height –––||–––
                    if (level[l, m] == null)
                    {
...
*/
        ...
        {
            if (roomNo < roomsInLevel.Length - 1)
            {//if its not the last room
...
*/
        for (int x = 0; x < MAX_LEVEL_WIDTH; x++)
        {
            for (int y = 0; y < MAX_LEVEL_HEIGHT; y++)
            {
                ...
                if ((x == MAX_LEVEL_WIDTH - 1 || x == 0) && level[x, y] != null)
                {//since the level array goes from 0-MAX_LEVEL_WIDTH-1, so will this.
                    level[x, y] = wall;
                }
                ...
                else
                {
                    //REST OF MAP
                    //vertical walls
                    if ((x != MAX_LEVEL_WIDTH - 1) && level[x, y] == null && (level[x + 1, y] != null && level[x + 1, y].type != Tile.tileType.wall))
                    {//If current position does not hold another tile and next position is not nothing and not a wall
                        level[x, y] = wall;
                    }
    ...
    public Quaternion rotateTowardsNearestTileOfType(Tile.tileType type, int x, int y, Tile[,] levelMatrix)
    {//will return what a given positions neighbors are
    ...
    bool isSpaceAvailableWithinRange(int range, Tile inputTile, Tile[,] levelMatrix, bool debugColorOn)
    {
        ...
        if (inputTile != null)
        {
            if (inputTile.x - range > 0 && inputTile.x + range < MAX_LEVEL_WIDTH && inputTile.y - range > 0 && inputTile.y + range < MAX_LEVEL_HEIGHT)
            {// out of bounds check
    ...
    int getAmountOfNeighbors(int num)
    {//OMG i actually remembered how to do recursion! This will return the amount of neighbors based on a given input range
        if (num == 0)
        {
            return num;
        }

...
/*        Transform roomCenter;
        for (int i = 0; i < roomsInLevel.Length; i++){
            ...
            levelMatrix[(int)roomsInLevel[i].center.x, (int)roomsInLevel[i].center.z].tileMesh.transform.parent = GameObject.Find ("ARZones").transform;
...
*/		//TRIGGER COLLIDERS FOR ROOMS
		for(int i=0; i<roomsInLevel.Length; i++){
			BoxCollider triggerZone = levelMatrix[(int)roomsInLevel[i].center.x, (int)roomsInLevel[i].center.z].tileMesh.gameObject.AddComponent<BoxCollider>();
			levelMatrix[(int)roomsInLevel[i].center.x, (int)roomsInLevel[i].center.z].tileMesh.parent = GameObject.Find("ARZones").transform;
			triggerZone.isTrigger = true;
			triggerZone.size = new Vector3(roomsInLevel[i].width, 6, roomsInLevel[i].height);
		}
